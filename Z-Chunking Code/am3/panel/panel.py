...
def initialize_machines(params):
    ...
    machine_bounds = SimulatorMath.calculate_bounds(bpy.data.objects['Robot Body 0'])
    machine_width = machine_bounds.x.max - machine_bounds.x.min
    if (machine_width == 0):
        print("0 width")

    model_bounds = SimulatorMath.calculate_bounds(bpy.data.objects[params.model_name])
